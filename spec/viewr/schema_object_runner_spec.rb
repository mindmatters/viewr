  ...
  describe '#run' do
    ...
    context 'called with a view that has dependencies' do
      context 'and all dependencies have already run' do
        before do
          allow(schema_object_runner).to receive(:find_by_names).and_return(Set.new)
          allow(foo).to receive(:send)
      ...
      context 'and dependencies have not yet been run' do
        it 'first runs the view depended upon and then the given view' do
          allow(schema_object_runner).to receive(:find_by_names).with(['foo']).and_return(Set.new([foo]))
          allow(schema_object_runner).to receive(:find_by_names).with([]).and_return(Set.new)
      ...
      context 'and dependencies have already been run' do
        before do
          allow(schema_object_runner).to receive(:find_by_names).with([]).and_return(Set.new)
          allow(foo).to receive(:send).with(:method).ordered
